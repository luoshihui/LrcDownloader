...
+ (void)getLyricsByTitle:(NSString *)Title getLyricsByArtist:(NSString *)Artist getLyricsBySongDuration:(double)duration complete:(void (^)(NSString *songName, NSString *singer, NSString *lrc))complete {
    ...
    NSString *url         = [[NSString stringWithFormat:@"http://lyrics.kugou.com/search?ver=1&man=yes&client=pc&keyword=%@&duration=%.0f", Title, songDuration] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    NSData   *json        = [RequestSender fetchRequest:url];
    NSError  *error       = nil;
    id       responseBody = [json mj_JSONObject];
    ...
    NSString *song   = [bestDict objectForKey:@"song"];
    NSString *singer = [bestDict objectForKey:@"singer"];
    
    complete(song, singer, lyrics);
}
